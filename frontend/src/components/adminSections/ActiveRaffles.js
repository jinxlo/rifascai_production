...
const ActiveRaffles = () => {
  ...
  const fetchRaffles = async () => {
    try {
      ...
      const response = await axios.get('https://rifascai.com/api/raffle/all', {
        headers: { 
  ...
  const handleModifyRaffle = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://rifascai.com/api/raffle/${selectedRaffle._id}`,
        updateData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleToggleStatus = async (raffleId, currentStatus) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `https://rifascai.com/api/raffle/${raffleId}`,
  ...
  const handleDeleteRaffle = async (raffleId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://rifascai.com/api/raffle/${raffleId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
