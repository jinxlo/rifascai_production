...
// Create a specific namespace for your raffle events
const raffleNamespace = io.of('/raffle');

// Add authentication middleware to the namespace
raffleNamespace.use(async (socket, next) => {
  try {
    const token =
...
raffleNamespace.on('connection', (socket) => {
  console.log(
    'Client connected to raffle namespace:',
    socket.id,
    'User:',
    socket.user?.email
  );
  ...
  socket.on('disconnect', (reason) => {
    console.log(
      'Client disconnected from raffle namespace:',
      socket.id,
      'Reason:',
      reason
    );
...
// Update the middleware to use the namespace
app.use((req, res, next) => {
  req.io = raffleNamespace;
  next();
});
...
app.use('/api/payments', paymentRoutes(upload, raffleNamespace));
app.use('/api/tickets', ticketsRoutes(raffleNamespace));
...
cron.schedule('*/5 * * * *', async () => {
  ...
  try {
    ...
    if (expiredTickets.length > 0) {
      const ticketNumbers = expiredTickets.map(
        (ticket) => ticket.ticketNumber
      );
      const userIds = [
        ...new Set(expiredTickets.map((ticket) => ticket.userId))
      ];
      ...
      raffleNamespace.emit('ticketsReleased', { tickets: ticketNumbers });
      ...
      userIds.forEach((userId) => {
        if (userId) {
          ...
          raffleNamespace.to(`user-${userId}`).emit('your_tickets_released', {
            tickets: userTickets,
