...
const testLogin = async () => {
  try {
    ...
    try {
      await axios.get('https://rifascai.com/api/auth/me');
    } catch (error) {
    ...
    console.log('URL:', 'https://rifascai.com/api/auth/login');
    console.log('Payload:', {
      email: process.env.ADMIN_EMAIL,
      password: process.env.ADMIN_PASSWORD
    });

    const response = await axios.post('https://rifascai.com/api/auth/login', {
      email: process.env.ADMIN_EMAIL,
      password: process.env.ADMIN_PASSWORD
    }, {
      headers: {
